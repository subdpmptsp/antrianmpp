<x-filament-panels::page>
    ...
    <div class="oq-page">
        ...
        @if ($activeTab === 'dashboard')
            <div class="oq-stats">
                <article><span>Reservasi hari ini</span><strong>{{ \App\Models\OnlineQueueReservation::query()->whereDate('service_date', today())->count() }}</strong><small>Semua layanan online</small></article>
                <article><span>Sudah check-in</span><strong>{{ \App\Models\OnlineQueueReservation::query()->whereDate('service_date', today())->where('status', 'checked_in')->count() }}</strong><small>Nomor telah diterbitkan</small></article>
        ...
        @elseif ($activeTab === 'settings')
            @php
                ...
                $verificationCount = \App\Models\OnlineQueueReservation::query()
                    ->whereNotNull('identity_verified_at')
                    ->whereYear('identity_verified_at', now('Asia/Jakarta')->year)
                    ->whereMonth('identity_verified_at', now('Asia/Jakarta')->month)
                    ->count();
                $verificationLogs = \App\Models\OnlineQueueReservation::query()
